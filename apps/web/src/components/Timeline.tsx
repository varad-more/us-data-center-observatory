...
export function Timeline({ entries }: { entries: TimelineEntry[] }) {
  if (entries.length === 0) {
    return (
      <p className="muted">
        No evidence has been recorded for this site yet. That means Helios has not found
        anything, not that nothing is happening.
      </p>
    );
  }
      ...
      {entries.map((entry, index) => (
        ...
        >
          ...
          <div className="timeline-title">
            {entry.title}
            {entry.evidence && <AssertionBadge assertion={entry.evidence.assertion_class} />}
            {entry.evidence?.is_standing_condition && (
              <StatusPill
                title="A statement about the current state of the world rather than a dated event. Standing conditions do not decay with age."
              >
                Standing condition
              </StatusPill>
            )}
  ...
  return (
    <>
      ...
      <div className="provenance">
        ...
        <span className="mono muted" title="SHA-256 of the exact bytes Helios retained">
          sha256:{evidence.source.content_sha256.slice(0, 12)}
        </span>
        {evidence.snippet_locator && (
          <span className="mono muted" title="Location of this fact within the source document">
...
function StageTransitionDetail({
  ...
  return (
    <div className="provenance">
      <span>
        {transition.from_stage_label ?? "Unclassified"} &rarr; {transition.to_stage_label}
      </span>
      {transition.detection_lag_days !== null && transition.detection_lag_days > 0 && (
        <span
          className="muted"
