...
PyMuPDF is an optional dependency (the ``documents`` extra), so it is imported
lazily. Importing it at module scope would make the whole connector package
unimportable on a default install, which is the common case for anyone working
on the API or the frontend.
"""
...
def extract_text_from_pdf(pdf_bytes: bytes) -> str:
    ...
    Raises:
        ModuleNotFoundError: If the ``documents`` extra is not installed.
    """
    try:
        import fitz
    except ModuleNotFoundError as exc:  # pragma: no cover - environment guard
        raise ModuleNotFoundError(
            "PDF parsing requires the 'documents' extra: pip install -e '.[documents]'"
        ) from exc
